...
class TestThreadsEndpoints:
    ...
    def test_get_threads_empty(self, client: httpx.Client):
    ...
    def test_create_thread_requires_user(self, client: httpx.Client, test_user: dict):
        ...
        assert created_thread["user_id"] == str(test_user["id"])
    ...
    def test_create_thread_invalid_user_id(self, client: httpx.Client, load_test_data):
    ...
    def test_create_thread_duplicate_date(self, client: httpx.Client, test_user: dict):
        """Test that creating threads with duplicate user_id+date fails."""
        thread_date = dt.date.today()
        thread_data = [{"user_id": str(test_user["id"]), "date": str(thread_date)}]
    ...
    def test_get_threads_by_ids(self, client: httpx.Client, test_thread: dict):
    ...
    def test_patch_threads(self, client: httpx.Client, test_thread: dict):
    ...
    def test_upsert_threads(self, client: httpx.Client, test_user: dict):
        """Test POST /api/latest/threads/upsert."""
        thread_date = dt.date.today() + dt.timedelta(days=2)
        upsert_data = [{"user_id": str(test_user["id"]), "date": str(thread_date)}]
    ...
    def test_delete_threads(self, client: httpx.Client, test_thread: dict):
