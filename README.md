...
I used to use a simple google-sheet for tracking short daily reflections, along with various metrics like my sleep quality, excercise, etc. 
This project is intended to replace this with a simple web-app. I have tried to build and deploy it in a way that ensures security while keeping it zero-cost. 
...
**Disclosure**: the react/vite code on the frontend was vibe-coded, I am not a front-end dev. 

## Docs

See the [docs](./docs/) directory for all documentation. An overview of the key pages is below:
...
Everything is a container, so to quickstart:
...
- **Frontend**: TypeScript, React, Vite, Tailwind CSS — _largely vibe coded_
- **Cloud deployment:**: GCP, Neon/Aiven
- **Auth**: [Google OAuth](https://developers.google.com/identity/protocols/oauth2)

### Planned next steps
- [x] "local-mode" in which (for local/offline dev) gcloud auth is bypassed
- basic deployment in GCP:
    - [x] dev deployment
    - [x] produciton deployment
